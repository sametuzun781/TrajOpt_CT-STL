...
from di_until_aug_solver import cpg_module
...
def cpg_solve(prob, updated_params=None, **kwargs):
    ...
    prob.constraints[12].save_dual_value(np.array(res.cpg_dual.d12).reshape((1, 5), order='F'))
    prob.constraints[13].save_dual_value(np.array(res.cpg_dual.d13).reshape(3))
    prob.constraints[14].save_dual_value(np.array(res.cpg_dual.d14).reshape(3))
...
def cpg_gradient(prob, gradient_sol_primal=None, gradient_sol_dual=None):
    ...
    if gradient_sol_primal is not None and gradient_sol_dual is not None:
        ...
        gradient_sol.primal = [0] * 247
        gradient_sol.dual = [0] * 319
        use_sol = False
    ...
    prob.param_dict['gf_dt_last'].gradient = np.array(pdelta.gf_dt_last).reshape((1, 54), order='F')
