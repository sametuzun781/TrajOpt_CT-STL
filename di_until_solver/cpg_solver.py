...
from di_until_solver import cpg_module
...
def cpg_solve(prob, updated_params=None, **kwargs):
    ...
    if updated_params is None:
        updated_params = ["w_tr", "f_dt_last", "gf_dt_last", "f_ct_last", "A_ct_last", "B_ct_last", "C_ct_last", "f_comp_last", "gf_comp_last", "X_last", "f_bar", "A_bar", "B_bar", "C_bar", "U_last"]
    ...
    prob.var_dict['dx'].save_value(np.array(res.cpg_prim.dx).reshape((7, 5), order='F'))
    prob.var_dict['du'].save_value(np.array(res.cpg_prim.du).reshape((3, 5), order='F'))
    prob.var_dict['nu'].save_value(np.array(res.cpg_prim.nu).reshape((7, 4), order='F'))
    prob.var_dict['X'].save_value(np.array(res.cpg_prim.X).reshape((7, 5), order='F'))
    prob.var_dict['U'].save_value(np.array(res.cpg_prim.U).reshape((3, 5), order='F'))
    ...
    prob.constraints[4].save_dual_value(np.array(res.cpg_dual.d4).reshape((7, 5), order='F'))
    prob.constraints[5].save_dual_value(np.array(res.cpg_dual.d5).reshape((3, 5), order='F'))
    prob.constraints[6].save_dual_value(np.array(res.cpg_dual.d6).reshape(1))
    prob.constraints[7].save_dual_value(np.array(res.cpg_dual.d7).reshape((1, 4), order='F'))
    ...
    prob.constraints[10].save_dual_value(np.array(res.cpg_dual.d10).reshape(3))
...
def cpg_gradient(prob, gradient_sol_primal=None, gradient_sol_dual=None):
    ...
    if gradient_sol_primal is not None and gradient_sol_dual is not None:
        ...
        gradient_sol.primal = [0] * 166
        gradient_sol.dual = [0] * 217
        use_sol = False
    ...
    prob.param_dict['gf_dt_last'].gradient = np.array(pdelta.gf_dt_last).reshape((1, 35), order='F')
    prob.param_dict['f_ct_last'].gradient = np.array(pdelta.f_ct_last).reshape(1)
    prob.param_dict['A_ct_last'].gradient = np.array(pdelta.A_ct_last).reshape((1, 28), order='F')
    prob.param_dict['B_ct_last'].gradient = np.array(pdelta.B_ct_last).reshape((1, 12), order='F')
    prob.param_dict['C_ct_last'].gradient = np.array(pdelta.C_ct_last).reshape((1, 12), order='F')
    prob.param_dict['f_comp_last'].gradient = np.array(pdelta.f_comp_last).reshape(1)
    prob.param_dict['gf_comp_last'].gradient = np.array(pdelta.gf_comp_last).reshape((1, 35), order='F')
    prob.param_dict['X_last'].gradient = np.array(pdelta.X_last).reshape((7, 5), order='F')
    prob.param_dict['f_bar'].gradient = np.array(pdelta.f_bar).reshape((7, 4), order='F')
    prob.param_dict['A_bar'].gradient = np.array(pdelta.A_bar).reshape((49, 4), order='F')
    prob.param_dict['B_bar'].gradient = np.array(pdelta.B_bar).reshape((21, 4), order='F')
    prob.param_dict['C_bar'].gradient = np.array(pdelta.C_bar).reshape((21, 4), order='F')
    prob.param_dict['U_last'].gradient = np.array(pdelta.U_last).reshape((3, 5), order='F')
