...
from di_always_stc_solver import cpg_module
...
def cpg_solve(prob, updated_params=None, **kwargs):
    ...
    prob.var_dict['dx'].save_value(np.array(res.cpg_prim.dx).reshape((9, 6), order='F'))
    prob.var_dict['du'].save_value(np.array(res.cpg_prim.du).reshape((3, 6), order='F'))
    prob.var_dict['nu'].save_value(np.array(res.cpg_prim.nu).reshape((9, 5), order='F'))
    prob.var_dict['X'].save_value(np.array(res.cpg_prim.X).reshape((9, 6), order='F'))
    prob.var_dict['U'].save_value(np.array(res.cpg_prim.U).reshape((3, 6), order='F'))
    prob.constraints[0].save_dual_value(np.array(res.cpg_dual.d0).reshape(9))
    prob.constraints[1].save_dual_value(np.array(res.cpg_dual.d1).reshape(9))
    prob.constraints[2].save_dual_value(np.array(res.cpg_dual.d2).reshape(9))
    prob.constraints[3].save_dual_value(np.array(res.cpg_dual.d3).reshape(9))
    prob.constraints[4].save_dual_value(np.array(res.cpg_dual.d4).reshape(9))
    prob.constraints[5].save_dual_value(np.array(res.cpg_dual.d5).reshape((9, 6), order='F'))
    prob.constraints[6].save_dual_value(np.array(res.cpg_dual.d6).reshape((3, 6), order='F'))
    ...
    prob.constraints[11].save_dual_value(np.array(res.cpg_dual.d11).reshape((1, 5), order='F'))
    prob.constraints[12].save_dual_value(np.array(res.cpg_dual.d12).reshape(3))
...
def cpg_gradient(prob, gradient_sol_primal=None, gradient_sol_dual=None):
    ...
    if gradient_sol_primal is not None and gradient_sol_dual is not None:
        ...
        gradient_sol.primal = [0] * 248
        gradient_sol.dual = [0] * 321
        use_sol = False
    ...
    prob.param_dict['A_ct_last'].gradient = np.array(pdelta.A_ct_last).reshape((1, 45), order='F')
    prob.param_dict['B_ct_last'].gradient = np.array(pdelta.B_ct_last).reshape((1, 15), order='F')
    prob.param_dict['C_ct_last'].gradient = np.array(pdelta.C_ct_last).reshape((1, 15), order='F')
    prob.param_dict['f_comp_last'].gradient = np.array(pdelta.f_comp_last).reshape(1)
    prob.param_dict['gf_comp_last'].gradient = np.array(pdelta.gf_comp_last).reshape((1, 54), order='F')
    prob.param_dict['f_dt_last'].gradient = np.array(pdelta.f_dt_last).reshape(2)
    prob.param_dict['gf_dt_last'].gradient = np.array(pdelta.gf_dt_last).reshape((2, 54), order='F')
    prob.param_dict['X_last'].gradient = np.array(pdelta.X_last).reshape((9, 6), order='F')
    prob.param_dict['f_bar'].gradient = np.array(pdelta.f_bar).reshape((9, 5), order='F')
    prob.param_dict['A_bar'].gradient = np.array(pdelta.A_bar).reshape((81, 5), order='F')
    prob.param_dict['B_bar'].gradient = np.array(pdelta.B_bar).reshape((27, 5), order='F')
    prob.param_dict['C_bar'].gradient = np.array(pdelta.C_bar).reshape((27, 5), order='F')
    prob.param_dict['U_last'].gradient = np.array(pdelta.U_last).reshape((3, 6), order='F')
