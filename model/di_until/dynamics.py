...
def dynamics(params: T.Dict[str, T.Any]) -> T.Dict[str, T.Any]:
    ...
    def f(x: jnp.ndarray, u: jnp.ndarray, tau: float) -> jnp.ndarray:
        ...
        dx = dx.at[6].set(0)
        dx = dx.at[6].add( params['w_states_theta'] * jnp.maximum(0, u[0]**2 + u[1]**2 - ( jnp.cos(params['theta']) * u[2] )**2 )**2 )
        dx = dx.at[6].add( params['w_states_T_max'] * jnp.maximum(0, u[0]**2 + u[1]**2 + u[2]**2 - params['T_max']**2 )**2 )
        dx = dx.at[6].add( params['w_states_v_max'] * jnp.maximum(0, x[3]**2 + x[4]**2 + x[5]**2 - params['v_max']**2 )**2 )
