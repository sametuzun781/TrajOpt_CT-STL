...
def params_fcn( 
    ...
    x_init = np.hstack(([-10.0, 0.0, 0.0], np.zeros(n_x - 3)))
    x_final = np.hstack(([6.0,  0.0, 0.0], np.zeros(n_x - 3)))
    ...
    return {
        ...
        "f_dt_dim":  3,
        "w_dt_fcn":  [1.0, 1.0, 1.0],
        "pen_dt_fcn": [penalty_fcn_list[1], 
                       penalty_fcn_list[1], 
                       penalty_fcn_list[1]],   # or whatever penalty list index you want
        "name_dt_fcn": [['evnt_vst_1', 'none'][0], 
                        ['evnt_vst_2', 'none'][0], 
                        ['evnt_vst_3', 'none'][0]],  # must match the shape your solver expects
        ...
        ###
        "name_comp_fcn": [["none"][0]],
        "ncvx_solver" : ['pl', 'pcx'][0],
        # -------------------------------------------
        # --------------- prox-convex ---------------
        # -------------------------------------------

        "p_w":             np.array([-8.0, -5.0, 0.0]),
        "p_w_2":           np.array([-6.0,  5.0, 0.0]),
        "p_w_3":           np.array([-4.0, -5.0, 0.0]),
        "r_w":             0.5,
        "r_w_2":           0.5,
        "r_w_3":           0.5,
